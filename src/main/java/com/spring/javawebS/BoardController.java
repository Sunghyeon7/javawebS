...
public class BoardController {
	...
	public String boardContentGet(HttpSession session,
		...
		// 댓글 가져오기(replyVOS) : 출력할때 1차 정렬이 groupId 오름차순, 2차정렬이 idx 오름차순
	...
	*/
	...
	public String boardUpdatePost(BoardVO vo,
		...
		if(!origVO.getContent().equals(vo.getContent())) {
			...
			vo.setContent(vo.getContent().replace("/resources/data/board/", "/resources/data/ckeditor/"));
			...
			// 이미지들의 모든 복사작업을 마치면, ckeditor폴더경로를 board폴더 경로로 변경한다.(/resources/data/ckeditor/ ===>> /resources/data/board/)
			vo.setContent(vo.getContent().replace("/resources/data/ckeditor/", "/resources/data/board/"));
		}
	...
	public String boardReplyUpdatePost(
		...
		System.out.println("replyIdx : " + idx);
		BoardReplyVO replyVO = boardService.getBoardReplyIdx(idx);
