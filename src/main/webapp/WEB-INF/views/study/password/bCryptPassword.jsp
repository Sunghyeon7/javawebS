...
<div class="container">
  ...
  <pre>
    ...
    - BCryptPassword Ender는 BCrypt해싱함수를 사용하여 비밀번호를 인코딩해주는 메소드와 사용자에 의해 제출된 비밀번호를
    	저장소에 저장된비밀번호와의 일치여부를 확인해주는 메소드로 제공한다.
    ...
    - 사용되는 메소드 ?
     ...
     2) machers(java.lang.CharSequence)
     		제출된 인코딩 되지 않는 패스워드의 일치여부를 판단하기위해 인코딩된 패스워드와 비교 판단 한다.
     		반환타입은 boolean이다.
  </pre>
  <hr/>
  <p>
    <input type="text" name="pwd" id="pwd" autofocus />
    <input type="button" value="bCryptPassword 암호화" onclick="bCryptPasswordCheck()" class="btn btn-success" />
