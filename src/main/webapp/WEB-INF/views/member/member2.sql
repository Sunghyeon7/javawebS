...
create table member2 (
  ...
  level     int default 1, 								/* 회원등급(0:관리자, 1:우수 회원, 2:정회원, 3:준 회원, (4:운영자)) */
  visitCnt  int default 0,								/* 총 방문횟수 */
  startDate datetime default now(),				/* 최초 가입일 */
  lastDate  datetime default now(),				/* 마지막 접속일 */
  todayCnt  int default 0,								/* 오늘 방문한 횟수 */
  primary key (idx),											/* 주키 : idx(고유번호) */
  unique key (mid)												/* 중복 방지 키 */
);
...
select chat.* from (select * from memberChat2 order by idx desc limit 50) chat order by idx asc;